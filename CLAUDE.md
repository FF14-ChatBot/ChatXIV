...
- **Docs stay in sync with code:** when a change affects behavior documented in `docs/README.md` or `frontend/src/README.md`, update the doc in the same change rather than leaving it stale. New env vars need `backend/.env.example` and `environment.d.ts` updated together with the code. This also covers `docs/tasks/**/README.md` implementation-status tables — when a PR ships, advances, or invalidates a tracked item (a doc's status moving toward Done, a new gap opening, a milestone landing), update the relevant status table in the same PR rather than letting it drift until a separate refresh pass. These files are git-crypt encrypted and always render as binary on GitHub (see below) — that's expected, not a reason to skip the update: edit them normally in the IDE, where the working tree holds real plaintext, and review the change locally or via a decrypted diff pasted into chat rather than GitHub's PR view.
